 ...
 */
function band_booking_global_preprocess_html(&$vars) {
  // Get adminimal folder path.
  $band_booking_global_path = drupal_get_path('theme', 'band_booking_global');

  // Add default styles, to fight with Adminimal Theme, Had weight 1000
  drupal_add_css($band_booking_global_path . '/css/custom.css', array('group' => CSS_THEME, 'media' => 'all', 'weight' => 1000));
}

// theme_select
function band_booking_global_select($variables) {
    $element = $variables['element'];
    element_set_attributes($element, array('id', 'name', 'size'));
    _form_set_class($element, array('form-select'));
    return '<select' . drupal_attributes($element['#attributes']) . '>' . band_booking_global_form_select_options($element) . '</select>';
}


function band_booking_global_form_select_options($element, $choices = NULL) {
    if (!isset($choices)) {
